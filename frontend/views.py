...
def get_tags():
    tags = Tag.objects.values("tag_content").annotate(num=Count('tag_content')).order_by('-num')[:20]
    return tags

def sound_list(request):
    tags = get_tags()
    sound_list = UserSound.objects.filter(is_approved="Yes").order_by('-upload_time')
    paginator = Paginator(sound_list,5)
    page = request.GET.get('page')
    sounds = paginator.get_page(page)
...
def production_list(request):
    tags = get_tags()
    prod_list = Production.objects.filter(is_approved="Yes").order_by('-upload_time')
...
def date_filter(request, arg):
    ...
    sounds = UserSound.objects.filter(is_approved="Yes",upload_time__gte=date_start,upload_time__lt=date_end)
...
def upload_production(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            upload.audio_file.name = handle_upload(settings.PROD_DIR,form.cleaned_data['audio_file'])
...
def sound_upload(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            upload.upload_time = timezone.now()
            upload.audio_file.name = handle_upload(settings.SOUND_DIR,form.cleaned_data['audio_file'])
            upload.save()
...
def user_record(request):
    ...
    if request.method == 'POST':
        try:
            ...
            upload.audio_file.name = handle_upload(settings.SOUND_DIR,uploaded_file)
            upload.save()
